...
import { getAllUsers, updateSelf } from "../api/api";

export default function UserPage() {
  const { user, updateStoredCreds } = useAuth();
  ...
  const loadUsers = async () => {
    ...
    try {
      ...
      setListError("Could not load users (this endpoint may require admin access).");
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await updateSelf({ username, password: password || undefined });
      // Keep stored credentials in sync so future requests still authenticate
      updateStoredCreds({ username, password: password || user.password });
